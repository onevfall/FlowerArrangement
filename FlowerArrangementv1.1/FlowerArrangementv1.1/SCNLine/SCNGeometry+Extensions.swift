...
public struct GeometryParts {
  ...
  public var uvs: [CGPoint]   //二维点
  public var indices: [UInt32]   //A 32-bit unsigned integer value type
  func buildGeometry() -> SCNGeometry {
    let src = SCNGeometrySource(vertices: self.vertices)
    let normals = SCNGeometrySource(normals: self.normals)  //法线
    let textureMap = SCNGeometrySource(textureCoordinates: self.uvs)  //从纹理坐标点数组创建一个几何源
    let inds = SCNGeometryElement(indices: self.indices, primitiveType: .triangles)  //按照索引数组指定的顺序连接顶点
    return SCNGeometry(sources: [src, normals, textureMap], elements: [inds])
  }
}

private extension simd_quatf {   //一个单精度四元数
  func act(_ vector: SCNVector3) -> SCNVector3 {
    let vec = self.act(SIMD3<Float>([vector.x, vector.y, vector.z])) //返回被四元数旋转的指定向量
    return SCNVector3(vec.x, vec.y, vec.z)
  }
  func split(by factor: Float = 2) -> simd_quatf { //四元数旋转
    if self.angle == 0 {
      return self
    } else {
...
private func rotationBetween2Vectors(start: SCNVector3, end: SCNVector3) -> simd_quatf {
  return simd_quaternion(simd_float3([start.x, start.y, start.z]), simd_float3([end.x, end.y, end.z]))  //从两个三元素向量返回一个新的四元数。
...
public extension SCNGeometry {
  ...
  private static func getCircularPoints(
    ...
    let angleAdd = Float.pi * 2 / Float(edges) //把旋转一圈的360度分为edges份
...
//创建一条粗线在3D空间的一系列点
  ...
  ///   - maxTurning: Maximum number of additional points to be added on turns. Varies depending on degree change.  在转弯时增加的额外点的最大数量。随度数变化而变化。
